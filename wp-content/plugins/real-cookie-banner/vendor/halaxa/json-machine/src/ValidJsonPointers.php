...
use DevOwl\RealCookieBanner\Vendor\JsonMachine\Exception\InvalidArgumentException;
/** @internal */
    ...
    private function validateJsonPointersDoNotIntersect()
    {
        foreach ($this->jsonPointers as $keyA => $jsonPointerA) {
            foreach ($this->jsonPointers as $keyB => $jsonPointerB) {
                ...
                if ($jsonPointerA === $jsonPointerB || self::str_contains("{$jsonPointerA}/", "{$jsonPointerB}/") || self::str_contains("{$jsonPointerA}/", self::wildcardify("{$jsonPointerB}/"))) {
                    throw new InvalidArgumentException(\sprintf("JSON Pointers must not intersect. At least these two do: '%s', '%s'", $jsonPointerA, $jsonPointerB));
                }
            }
        }
    }
